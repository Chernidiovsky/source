    ...
    ("reports", 0,
     ...
     [
         ...
         ("action_view_world_map", [], "View the world map.",
          [
              (start_presentation, "prsnt_world_map_new"),
          ]
          ),
 ...
 You want everything, and you won't let anyone stop you from having it..."),
    ...
    ("morale_report", 0,
     ...
     [
         ...
         (try_end),
         (str_store_string, s1,
          "@Current party morale is {reg5}.^Current party morale modifiers are:^^Base morale:  +50^Party size: {s2}{reg1}^Leadership: {s3}{reg2}^Player's level: +{reg9}^Food variety: {s4}{reg3}{s5}{s6}^Recent events: {s7}{reg4}^TOTAL:  {reg5}^^Morale value to change: {s8}{reg8}^Prospective party morale: {reg10}^^^"),
         ## CC
    ...
    (
        ...
        [
            ...
            ("change_commander_simple_encounter",
             ...
             "Change commander.(Current is {s7})",
             [
...
{reg1? You may station troops here to defend it against enemies who may try to recapture it. Also, you should select now whether you will hold the {reg2?town:castle} yourself or give it to a faithful vassal...:}",  # Only visible when castle is taken without being a vassal of a kingdom.
        ...
        [
            ...
            ("Meet_with_village_elder",
             [
                 ...
                 (str_clear, s20),
                 (try_begin),
                 (troop_slot_eq, ":town_elder", slot_troop_met, 0),
                 (str_store_string, s20, "@(Met once to unlock)"),
                 (disable_menu_option),
                 (try_end),
             ],
             "Meet with the elder{s20}.",
             [
                 (try_begin),
    ...
    (
        ...
        [
            ...
            ("meet_with_guild_master",
             ...
             "Meet with the guild master{s20}.",
             [
                 (party_get_slot, ":conversation_troop", "$current_town", slot_town_elder),
                 (call_script, "script_setup_troop_meeting", ":conversation_troop", -1),
             ]),
 ...
 The bandit parties will come to accompany them. They become much united, not like a sheet of loose sand.", "none", [],
    ...
    (
        ...
        "The following soldiers want to join you. Total recruiting cost is {reg1} denars.^^{s12}",
        "none",
        [
            (call_script, "script_cf_generate_temp_party_for_player_center_recruiting", "$current_town"),
            (call_script, "script_get_party_join_cost", "p_temp_party"),
            (assign, ":party_join_cost", reg0),
            (assign, reg1, ":party_join_cost"),
            (call_script, "script_print_party_to_s0", "p_temp_party"),
            (str_store_string_reg, s12, s0),
            (party_get_free_companions_capacity, ":free_capacity", "p_main_party"),
            (try_begin),
            (this_or_next | le, ":free_capacity", 0),  # not enough room
            (le, "$temp_3", 0),  # recruit chances is out
            (party_clear, "p_temp_party"),
            (jump_to_menu, "mnu_town"),
            (try_end),
        ],
        [
            ("no_money_to_recruit",
             [
                 (store_troop_gold, ":gold", "trp_player"),
                 (lt, ":gold", reg1),  # reg1 = party_join_cost
            ...
            ("recruit_to_your_party",
             [
                 (store_troop_gold, ":gold", "trp_player"),
                 (ge, ":gold", reg1),  # reg1 = party_join_cost
             ], "Recruit them to your party.",
             [
                 (troop_remove_gold, "trp_player", reg1),  # reg1 = party_join_cost
                 (party_set_flags, "p_main_party", pf_limit_members, 0),
